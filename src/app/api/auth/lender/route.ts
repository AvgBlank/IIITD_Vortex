...
export async function GET(request: NextRequest) {
  try {
    ...
    const user = await prisma.LenderUser.findUnique({
      where: { email },
    });
...
export async function POST(request: Request) {
  try {
    ...
    const checkEmail = await prisma.LenderUser.findUnique({ where: { email } });
    if (checkEmail) {
      return NextResponse.json(
    ...
    while (
      await prisma.LenderUser.findUnique({ where: { id: randomId.toString() } })
    ) {
      randomId = Math.floor(1000000000 + Math.random() * 9000000000);
    }

    const newUser = await prisma.LenderUser.create({
      data: {
